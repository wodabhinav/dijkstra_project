onload = function () {
  ...
  function createData() {
    // V = Math.floor(Math.random() * 12) + 3; // Ensures V is between 3 and 15
    V = 15;
    let nodes = [];
    for (let i = 1; i <= V; i++) {
  ...
  solve.onclick = function () {
    ...
    if (source.value == "none" || destination.value == "none") {
  ...
  function djikstra(graph, sz, src) {
    ...
    for (let i = 0; i < sz - 1; i++) {
      ...
      for (let j = 0; j < sz; j++) {
        if (vis[j] === 0) {
          if (mn === -1 || dist[j][0] < dist[mn][0]) mn = j;      // same like priority queue finding out the minimum distance
        }
      }
